...
In a rigorous walk-forward backtest on S\&P 500 constituents (2015--2025), the proposed framework achieves a \textbf{Sharpe Ratio of 1.38}, representing a \textbf{62\% improvement} over factor ranking baselines (0.85) and significantly outperforming empirical covariance methods (0.68). We further validate the model's realism using the \textbf{Predictive Score} metric, demonstrating a \textbf{3$\times$ improvement} over Generative Adversarial Networks (GANs) in capturing stylized facts such as volatility clustering and the leverage effect.
\end{abstract}
...
\hline
\textbf{Factor-DiT} & \textbf{1.38} & \textbf{16.2\%} & \textbf{-11.3\%} & \textbf{0.142} \\
Ranking Baseline & 0.85 & 11.8\% & -16.7\% & 0.089 \\
Empirical MVO & 0.68 & 9.2\% & -21.4\% & 0.072 \\
Buy \& Hold & 0.52 & 7.8\% & -26.1\% & 0.058 \\
\hline
\end{tabular}
\end{table}

As shown in Fig.~\ref{fig:sharpe}, the Factor-DiT strategy achieves a Sharpe Ratio of 1.38, a \textbf{62\% improvement} over the ranking baseline (0.85) and a \textbf{103\% improvement} over empirical MVO (0.68). The reduction in Max Drawdown (-11.3\% vs -16.7\%) demonstrates superior risk management through the diffusion-generated covariance matrix.
...
By modeling log-prices via GBM-inspired SDEs, we achieve two key theoretical properties:
\begin{enumerate}
\item \textbf{Heteroskedasticity:} Volatility scales proportionally with the current price level, implementing multiplicative rather than additive noise.
\item \textbf{Heavy Tails:} The VE-SDE formulation naturally generates non-Gaussian return distributions with empirically realistic tail exponents of $\alpha \approx 4$.
\end{enumerate}
...
\textbf{Results:}
\begin{itemize}
\item Raw Sharpe: 1.38
\item Deflated Sharpe: \textbf{1.18}
\item p-value: 0.008 (significant at 1\% level)
\end{itemize}

As shown in Fig.~\ref{fig:deflated}, the deflated Sharpe (1.18) remains well above the significance threshold (1.0), confirming our results are not due to data dredging.
...
To ensure temporal independence, we employed \textbf{purged k-fold cross-validation} with a 10-day purge window on both sides of each test fold. The mean Sharpe across 5 folds was 1.32 (std: 0.11), confirming that our results are not artifacts of a single favorable train/test split.

\subsection{Execution Latency Mitigation}
A critical concern is that diffusion sampling (500 samples $\times$ 100 steps) requires $\sim$30 seconds on an A100 GPU. In live trading, this latency could result in slippage that erodes alpha.

\textbf{Production Deployment Strategy:} In our backtest, we assume the model runs at 15:30 ET (30 minutes before market close), pre-computing next-day portfolio weights. Orders are then staged and executed at 16:00 via VWAP algorithms. This eliminates real-time latency entirely.

\begin{table}[!htbp]
\centering
\caption{Impact of Execution Timing on Net Performance}
\label{tab:execution}
\begin{tabular}{lccc}
\hline
\textbf{Strategy} & \textbf{Exec. Time} & \textbf{Slippage} & \textbf{Net Sharpe} \\
\hline
Real-time Lag & 16:00:30 & 4.2 bps & 1.21 \\
\textbf{Pre-computed} & \textbf{15:30} & \textbf{2.1 bps} & \textbf{1.34} \\
Overnight (T+1) & 09:30:00 & 8.7 bps & 0.98 \\
\hline
\end{tabular}
\end{table}

As shown in Table~\ref{tab:execution}, pre-computation reduces slippage from 4.2 to 2.1 basis points, preserving 97\% of gross alpha (1.34 vs 1.38 Sharpe). For intraday strategies with holding periods $<1$ day, the 30-second latency remains prohibitive. However, for daily rebalancing, it is a non-issue.

\subsection{Covariance Stability in Crisis Regimes}
Mean-Variance Optimization is notoriously sensitive to estimation error (\textit{Markowitz's Curse}). We validate that the diffusion-generated covariance matrix $\Sigma$ does not ``explode'' during high-volatility regimes.

We compute the \textbf{condition number} $\kappa(\Sigma) = \lambda_{\max}/\lambda_{\min}$ for both empirical and diffusion-generated covariance matrices across three regimes:

\begin{table}[!htbp]
\centering
\caption{Covariance Matrix Condition Number by Regime}
\label{tab:condition}
\begin{tabular}{lccc}
\hline
\textbf{Regime} & \textbf{Empirical $\Sigma$} & \textbf{Diffusion $\Sigma$} & \textbf{Reduction} \\
\hline
Normal (VIX $<20$) & 142.3 & 38.7 & 73\% \\
High Vol (VIX 20-30) & 389.2 & 52.1 & 87\% \\
\textbf{Crisis (VIX $>30$)} & \textbf{1,247.5} & \textbf{94.3} & \textbf{92\%} \\
\hline
\end{tabular}
\end{table}

\textbf{Key Result:} During the March 2020 COVID crash (VIX $>80$), the empirical covariance matrix had a condition number exceeding 1,200, leading to extreme MVO weight instability. The diffusion model, by learning the low-rank structure of returns, acts as a \textit{natural regularizer}, keeping $\kappa(\Sigma) < 100$ even in crisis regimes.

This explains why Factor-DiT outperforms empirical MVO by 103\% (1.38 vs 0.68 Sharpe)---the performance gap widens during volatility spikes, precisely when accurate covariance estimation is most critical.
...
The transition from heuristic ranking to Generative Mean-Variance Optimization unlocks the ability to estimate future covariance structures. The resulting strategy delivers a Sharpe Ratio of 1.38, driven by superior risk diversification and cost-efficient execution.
