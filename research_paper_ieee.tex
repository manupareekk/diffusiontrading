...
In a rigorous walk-forward backtest on S\&P 500 constituents (2015--2025), the proposed framework achieves a \textbf{Sharpe Ratio of 1.38}, significantly outperforming empirical baselines (0.96) and heuristic ranking methods (1.15). We further validate the model's realism using the \textbf{Predictive Score} metric, demonstrating a \textbf{3$\times$ improvement} over Generative Adversarial Networks (GANs) in capturing stylized facts such as volatility clustering and the leverage effect.
\end{abstract}
...
Ranking Baseline & 1.15 & 14.1\% & -14.2\% & 0.125 \\
...
As shown in Fig.~\ref{fig:sharpe}, the Factor-DiT strategy achieves a Sharpe Ratio of 1.38, a 20\% improvement over the ranking baseline (1.15). The reduction in Max Drawdown (-11.3\% vs -14.2\%) demonstrates superior risk management through the diffusion-generated covariance matrix.
